class KonvaSketch {
    constructor({
        ...
        this.property_id = this.container.find('#property_id');
        this.property_type = this.container.find('#property_type');
        this.property_parent = this.container.find('#property_parent');
        this.property_name = this.container.find('#property_name');
        this.unused_percent = this.container.find('#unused_percent');
        this.corridor_percent = this.container.find('#corridor_percent');
        ...
        this.btn_zoomin.on('mousedown', () => this.sketch_canvas_zoom(0.5));
        ...
        this.btn_rotate = this.container.find('#btn_rotate');
        this.btn_rotate.off('click');
        this.btn_rotate.on('click', () => this.rotate_node());
    ...
    rotate_node() {
        if(this.selected) {
            this.get_children(this.selected).forEach(node => this.rotate_one(node));
            this.rotate_one(this.selected);
            this.save_history();
        }
    }
    on_width_change(e) {
    ...
    undo_history() {
        // this.container.find('.floor_sets').remove();
    ...
    display_property(node) {
        if (node === this.stage) {
            ...
            this.property_id.val(node.id());
            this.property_type.val(node.name());
            this.property_name.val(node.name());
            
            //*-------------------Sven Volger(2024-01-12)------------------------*//
            if(node.name().includes('building') || node.name().includes('external')) this.property_parent.val(this.plot.id());
            else if(node.name().includes('plot') || node.name().includes('road') || node.name().includes('neighbor')) this.property_parent.val('Null');
            else {
                var txt = node.id();
                console.log(node.id());
                var index = 0;
                if(txt.includes("_stack_")) index = txt.indexOf("_stack_");
                else index = txt.indexOf("_floor_");

                this.property_parent.val(txt.slice(0,index));
            }
            //*------------------------------------------------------------------*//

            //*---------------------area-percentage-------------------------*//
            this.unused_percent.text("1%");
            this.corridor_percent.text("1%");
            var total_area = this.width * this.height;
            var plot_area = this.plot.width() * this.plot.height();
            var corridor_area = 0;
            this.layer.find('.primary').forEach(node => {
                if(!node.hasName('plot')) {
                    corridor_area += node.width() * node.height();
                }
            });
            var unused_area = total_area - plot_area - corridor_area;

            this.unused_percent.text((unused_area / total_area * 100).toFixed(2) + "%");
            this.corridor_percent.text((corridor_area / total_area * 100).toFixed(2) + "%");
